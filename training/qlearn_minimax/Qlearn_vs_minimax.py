from agents.qplayerandothers.players import DqnAgent
...
for episode in range(EPISODES):
    ...
    try:
        ...
        statistical_data[episode] = [f"bad-moves: {game.player2.bad_moves}", f"good-moves: {game.player2.good_moves}", f" won: {isinstance(winner, DqnAgent)}"]
        statistical_data_minimax[episode] = [f"bad-moves: {game.player1.bad_moves}", f"good-moves: {game.player1.good_moves}",
                                     f"won: {isinstance(winner, RandomPlayer)}"]
